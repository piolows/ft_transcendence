...
export default class Tournament extends Component {
    ...
    private tournamentItem(roomName: String, playerCount: Number, maxPlayers: Number) {
        ...
        return `
        <div class="tournament-item grid grid-cols-2">
    ...
    private async getTournamentData(): Promise<any> {
        ...
        // {
        //     const [tournamentId, tournamentData] = tournaments[i];
        //     this.items.push(tournamentData);
        // }
    }
    ...
    async load (app: HTMLDivElement | HTMLElement) {
        ...
        for (let i = 0, length = Object.keys(tournaments).length; i < length; i++) {
            console.log('item:', tournaments[i]);
            const tournamentItem = tournaments[i] as any;
            html += this.tournamentItem(tournamentItem.roomName, Object.keys(tournamentItem.players).length !== 0 ? tournamentItem.players.length : 0, tournamentItem.maxPlayers);
        }
        app.innerHTML += html;
        // load the navbar 
    //     app.innerHTML += `
    ...
    //             ${this.tournamentItem("Emad's room", 1, 16)}
    //             ${this.tournamentItem("Piolo's room", 16, 16)}
    ...
    async init() {
        console.log(`THe tournament page init function is being called`);
        this.navbar.init();
    }
